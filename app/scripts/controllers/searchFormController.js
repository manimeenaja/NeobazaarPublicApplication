...
angular.module('Neobazaar')
  .controller('SearchFormController', function($scope, $rootScope, $location, $routeParams) {
	  ...
	  $scope.$watch('locations', function()  {
		  $scope.params.location = 
			  $rootScope.getCurrentElement($rootScope.locations, 'value', $routeParams.location);
	  });
	  
	  $scope.$watch('categories', function()  {
		  $scope.params.category = 
			  $rootScope.getCurrentElement($rootScope.categories, 'id', $routeParams.category);
	  });
	  ...
	  $scope.doSearch = function() {
		  for(var i in $scope.params) {
			  if(-1 !== $.inArray(i, ['location', 'category', 'purpose'])) continue;
			  params[i] = $scope.params[i];
		  }
		  $location.path('annunci-' + $scope.params.location.value + '/' + 
				  $scope.params.purpose.value + '/' + $scope.params.category.id).search(params);
	  }
