...
angular.module('Neobazaar')
  .controller('BreadcrumbsController', function($scope, $routeParams) {
	  ...
	  $scope.getBreadcrumbs = function(params) {
		  	...
		  	var locationFormatted = null !== location ? location.charAt(0).toUpperCase() + location.substr(1) : '';
		  	var voiceOne = null !== purpose ? locationFormatted + ', ' + purpose : locationFormatted;
		  	var voiceTwo = null !== query ? query + ' in ' + category : category;
		  	
		  	var voiceTwoUrl = null !== query
		  		? '#/annunci-' + location + '/' + purpose + '/' + page + '?query=' + query 
		  		: '#/annunci-' + location + '/' + purpose + '/' + page
		  	
			return [
		        {label: 'Home', link: '#/'},
			    {label: 'Tutti gli annunci', link: '#/annunci-italia'},
			    {label: voiceOne, link: '#/annunci-' + location + '/' + purpose},
			    {label: voiceTwo, link: voiceTwoUrl},
	        ];
	  }
