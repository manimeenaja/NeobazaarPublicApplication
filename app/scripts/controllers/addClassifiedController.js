...
//             }
  ...
  .controller('ImageUploadController', function ($scope, $rootScope, 
		  $routeParams, $parse, $timeout, $location, ClassifiedLoader) {
	  ...
	  if('id' in $routeParams && 40 == ($routeParams.id).length) {
		  ...
		  $scope.classifiedResource.then(function(response) {	
			  ...
			  $scope.classified.user.name = 'just some data'; // non viene preso in considerazione
			  $scope.classified.user.email = 'dummy@user.com'; // non viene preso in considerazione
			  $scope.classified.user.id = data.userid;
			  $scope.classified.title = data.title;
			  $scope.classified.location = 
				  $rootScope.getCurrentElement($rootScope.locations, 'value', data.location);
			  $scope.classified.category = 
				  $rootScope.getCurrentElement($rootScope.categories, 'id', data.category);
			  ...
			  $scope.populateMetadata = function(scope, rootScope, metadata) {
				  for(var i in metadata) {
					 ...
					 if(null === data) {
						 ...
						 scope.classified[metamap] = 
							 rootScope.getCurrentElement(rootScope[metamap], 'value', metadata[i].value);
					 }
				  }	
			  }
  ...
  .controller('AddClassifiedController', function ($scope, $rootScope, $routeParams, Classified) {
	  $scope.options = {
    		  url: url
      }; 
	  ...
	  $scope.successfull = function(data, status, headers, config) {
		  $('#classified').unblock();
		  try {
	  ...
	  $scope.isInvalidCategory = function() {
		    if ($scope.classified.category.value) return false;
		    return true;
	  }
	  ...
	  $scope.activate = function(classified) {
		  ...
		  Classified.disableEnable({'id':classified.id}, function(data) {
				if('status' in data && 'success' == data.status) {
	  ...
	  $scope.prepareRequestParams = function(scope) {
		  $('#classified').block({ message: '<div>Loading</div>' });
		  ...
		  var categoryMetadata = new Array();
		  var c = null !== scope.classified.category ? scope.classified.category.value : '0';
		  switch(c) {
		  	case '26': // auto
		  		categoryMetadata = [{
		  			key: 'tipo_contratto',
		  			value: 'contracttype' in scope.classified && null != scope.classified.contracttype
		  				? scope.classified.contracttype.value : ''
		  		}, {
		  			key: 'comune',
		  			value: 'comune' in scope.classified && null != scope.classified.comune
		  				? scope.classified.comune.value : ''
		  		}, {
		  			key: 'cambio',
		  			value: 'cambio' in scope.classified && null != scope.classified.cambio
		  				? scope.classified.cambio.value : ''
		  		}, {
		  			key: 'tipo_auto',
		  			value: 'cartype' in scope.classified && null != scope.classified.cartype
		  				? scope.classified.cartype.value : ''
		  		}, {
		  			key: 'kilometri',
					value: 'kilometri' in scope.classified ? scope.classified.kilometri : ''
		  		}, {
		  			key: 'marca_auto',
		  			value: 'carbrand' in scope.classified && null != scope.classified.carbrand
		  				? scope.classified.carbrand.value : ''
		  		}, {
		  			key: 'indirizzo',
					value: 'address' in scope.classified && null != scope.classified.address
						? scope.classified.address : ''
		  		}, {
		  			key: 'carburante',
		  			value: 'fuel' in scope.classified && null != scope.classified.fuel 
		  				? scope.classified.fuel.value : ''
		  		}, {
		  			key: 'riferimento',
					value: 'reference' in scope.classified ? scope.classified.reference : ''
		  		}, {
		  			key: 'condizioni_auto',
		  			value: 'carcondition' in scope.classified && null != scope.classified.carcondition 
		  				? scope.classified.carcondition.value : ''
		  		}, {
		  			key: 'classe_euro',
		  			value: 'euroclass' in scope.classified && null != scope.classified.euroclass 
		  				? scope.classified.euroclass.value : ''
		  		}, {
		  			key: 'civico',
		  	...
		  	case '27': // moto e scooter
		  		categoryMetadata = [{
		  			...
		  			key: 'marca_moto',
		  			value: 'motobrand' in scope.classified && null != scope.classified.motobrand
		  				? scope.classified.motobrand.value : ''
		  		}, {
		  			key: 'condizioni_moto',
		  			value: 'motocondition' in scope.classified && null != scope.classified.motocondition 
		  				? scope.classified.motocondition.value : ''
		  		}, {
		  			key: 'comune',
		  			value: 'comune' in scope.classified && null != scope.classified.comune
		  				? scope.classified.comune.value : ''
					...
					value: 'civic' in scope.classified ? scope.classified.civic : ''
		  			...
		  			key: 'cilindrata',
		  			value: 'displacement' in scope.classified && null != scope.classified.displacement
		  				? scope.classified.displacement.value : ''
		  		}, {
		  			key: 'riferimento',
					value: 'reference' in scope.classified ? scope.classified.reference : ''
		  		}, {
		  			...
		  			key: 'tipo_moto',
		  			value: 'mototype' in scope.classified && null != scope.classified.mototype
		  				? scope.classified.mototype.value : ''
		  		}, {
		  			key: 'carburante',
		  			value: 'fuel' in scope.classified && null != scope.classified.fuel 
		  				? scope.classified.fuel.value : ''
		  	...
		  	case '106': // nautica
		  		categoryMetadata = [{
		  			...
		  			key: 'tipo_barca',
		  			value: 'boattype' in scope.classified && null != scope.classified.boattype
		  				? scope.classified.boattype.value : ''
		  		}, {
		  			key: 'carburante',
		  			value: 'fuel' in scope.classified && null != scope.classified.fuel 
		  				? scope.classified.fuel.value : ''
		  	...
		  	case '118': // caravan e camper
		  		categoryMetadata = [{
		  			...
		  			key: 'tipo_caravan',
		  			value: 'campertype' in scope.classified && null != scope.classified.campertype
		  				? scope.classified.campertype.value : ''
		  		}, {
		  			key: 'carburante',
		  			value: 'fuel' in scope.classified && null != scope.classified.fuel 
		  				? scope.classified.fuel.value : ''
		  	...
		  	case '35': // uffici e locali commerciali
		  		categoryMetadata = [{
		  			...
		  			key: 'numero_stanze',
					value: 'room' in scope.classified && null != scope.classified.room
						? scope.classified.room.value : ''
		  		}, {
		  			key: 'metri_quadri',
					...
					value: 'civic' in scope.classified ? scope.classified.civic : ''
		  			...
		  			key: 'box',
					value: 'carbox' in scope.classified && null != scope.classified.carbox
						? scope.classified.carbox.value : ''
		  		}, {
		  			key: 'boxmq',
					...
					value: 'constructionyear' in scope.classified ? scope.classified.constructionyear : ''
		  			...
		  			key: 'giardino',
					value: 'garden' in scope.classified && null != scope.classified.garden
						? scope.classified.garden.value : ''
		  		}, {
		  			key: 'giardinomq',
					...
					value: 'realestateunit' in scope.classified ? scope.classified.realestateunit : ''
		  			...
		  			key: 'piano',
					value: 'floor' in scope.classified && null != scope.classified.floor
						? scope.classified.floor.value : ''
		  		}, {
		  			key: 'totale_piani',
					...
					value: 'montlyexpenses' in scope.classified ? scope.classified.montlyexpenses : ''
		  			...
		  			key: 'condizioni',
					value: 'condition' in scope.classified && null != scope.classified.condition
						? scope.classified.condition.value : ''
		  		}, {
		  			key: 'rogito',
					value: 'deed' in scope.classified && null != scope.classified.deed
						? scope.classified.deed.value : ''
		  		}, {
		  			key: 'riferimento',
					value: 'reference' in scope.classified ? scope.classified.reference : ''
		  		}, {
					...
					value: 'lift' in scope.classified ? scope.classified.lift : ''
		  			...
		  			key: 'riscaldamento',
					value: 'heating' in scope.classified && null != scope.classified.heating
						? scope.classified.heating.value : ''
		  		}, {
		  			key: 'numero_bagni',
					value: 'bathroom' in scope.classified && null != scope.classified.bathroom
						? scope.classified.bathroom.value : ''
		  		}];
		  		break;
		  	case '32': // camera, posto letto
		  		categoryMetadata = [{
		  			...
		  			key: 'indirizzo',
					value: 'address' in scope.classified && null != scope.classified.address
						? scope.classified.address.value : ''
		  		}, {
		  			key: 'civico',
					value: 'civic' in scope.classified ? scope.classified.civic : ''
		  		}, {
		  			key: 'fumatori',
					value: 'smoker' in scope.classified && null != scope.classified.smoker
						? scope.classified.smoker.value : ''
		  		}, {
		  			key: 'tipo_stanza',
					value: 'roomtype' in scope.classified && null != scope.classified.roomtype
						? scope.classified.roomtype.value : ''
		  		}, {
		  			key: 'affittasi_a',
					value: 'rentto' in scope.classified && null != scope.classified.rentto
						? scope.classified.rentto.value : ''
		  		}, {
		  			key: 'riferimento',
					value: 'reference' in scope.classified ? scope.classified.reference : ''
		  		}, {
		  	...
		  	case '62': // cerco lavoro
		  		categoryMetadata = [{
		  			...
		  			value: 'jobcontracttype' in scope.classified && null != scope.classified.jobcontracttype
		  				? scope.classified.jobcontracttype.value : ''
		  		}, {
		  			key: 'orario_lavoro',
		  			value: 'workinghours' in scope.classified && null != scope.classified.workinghours
		  				? scope.classified.workinghours.value : ''
		  		}, {
		  			key: 'inquadramento_lavoro',
		  			value: 'jobtype' in scope.classified && null != scope.classified.jobtype
		  				? scope.classified.jobtype.value : ''
		  		}];
		  		break;
		  }
